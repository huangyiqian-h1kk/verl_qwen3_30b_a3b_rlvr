#!/bin/bash
# Reasoning Gym 16K training snapshots: original v3, two checkpoint-partitioned PBS jobs.
# Fixed full steps: A=30,40,50,60,70; B=80,90,100,110,120.
# 16K names the training phase; legal generation remains 4096 tokens by default.
...
MODE=${MODE:-full}
PARTS=${PARTS:-both}
LEGAL_DATA=${LEGAL_DATA:/groups/gcg51557/experiments/0390_rlsd/RLVR/verl_qwen3_30b_a3b_rlvr/data/remain_all_cases.jsonl}
SNAPSHOT_ROOT=${SNAPSHOT_ROOT:-$PROJ/ckpts/model_snapshots/inst2507_rg_sixcat_r08_k8_full_v1_len16k}
...
case "$MODE" in
  smoke)
    ...
    OUT_ROOT=${OUT_ROOT:-$PROJ/outputs/legal_entropy_curve_qwen3_30b_reasoning_gym_16k_smoke_unicode}
    JOB_NAME_A=0390_rg16k_sm_a
    JOB_NAME_B=0390_rg16k_sm_b
    ;;
  full)
    SAMPLE_SIZE=${SAMPLE_SIZE:-100}
    STEP_LIST_A=30:40:50:60:70
    STEP_LIST_B=80:90:100:110:120
    PROBE=${PROBE:-$PROJ/data/legal_entropy_probe/legal_probe_n${SAMPLE_SIZE}_seed${PROBE_SEED}.jsonl}
    OUT_ROOT=${OUT_ROOT:-$PROJ/outputs/legal_entropy_curve_qwen3_30b_reasoning_gym_16k_unicode_none}
    JOB_NAME_A=0390_rg16k_fl_a
    JOB_NAME_B=0390_rg16k_fl_b
    ;;
  *)
...
MANIFEST=$OUT_ROOT/submitted_reasoning_gym_16k_2jobs_$(date +%F_%H%M%S).txt
...
submit_part() {
  ...
  job_id=$(
    qsub \
      ...
      -o "$pbs_log" \
      -v "$variables" \
      "$PROJ/jobs/q3_legal_rlvr_entropy_curve_reasoning_gym_16k_part_v3.pbs"
  )
...
echo "[NEXT] after both parts finish, run finalize_legal_rlvr_entropy_curve_reasoning_gym_16k_2jobs_v3.sh"
